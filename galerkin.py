...
class FunctionSpace:
    ...
    def inner_product(self, u): ## for calculating right hand side inner prod
...
class NeumannLegendre(Composite, Legendre):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        ...
        ones = np.ones(N+1)
        alpha = [-i*(i+1)/(i+2)/(i+3) for i in range(N+1)]
        self.S = sparse.diags([ones, alpha], [0, 2], shape=(N+1, N+3), format='csr')
...
class NeumannChebyshev(Composite, Chebyshev):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        ...
        ones = np.ones(N+1)
        alpha = [-((i/(i+2))**2) for i in range(N+1)]
        self.S = sparse.diags([ones, alpha], [0, 2], shape=(N+1, N+3), format='csr')
